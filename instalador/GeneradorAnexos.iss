...
#ifdef ExigirVcRedist
  #ifndef IncluyeVcRedist
    #error La distribución requiere VC_redist.x64.exe verificado. Ejecute scripts/preparar-vcredist.ps1.
  #endif
#endif
...
; Compresión acotada: ultra64 con varios bloques agota la memoria del compilador.
...
Source: "{#VcRedistArchivo}"; Flags: dontcopy
#endif
...
Filename: "{app}\{#MiEjecutable}"; \
    Flags: nowait runasoriginaluser; Check: InstalacionSilenciosa
...
begin
  ...
  if not Exec(ExpandConstant('{tmp}\VC_redist.x64.exe'),
      '/install /quiet /norestart', '', SW_HIDE, ewWaitUntilTerminated, Codigo) then
  begin
    Result := 'No se pudo instalar Microsoft Visual C++ x64. Intente nuevamente.';
    exit;
  end;
  { 1638: ya existe una versión más reciente. 3010: reinicio pendiente. }
  if (Codigo <> 0) and (Codigo <> 3010) and (Codigo <> 1638) then
  begin
    Result := 'Microsoft Visual C++ no pudo instalarse (código ' + IntToStr(Codigo) + ').';
    exit;
  end;
  if Codigo = 3010 then NeedsRestart := True;
  if not VcRedistInstalado then
    Result := 'No se pudo verificar Microsoft Visual C++ x64. No se copiará la aplicación.';
#endif
end;
