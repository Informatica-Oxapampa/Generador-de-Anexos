...
echo.
echo [1/5] Buscando el SDK de .NET 8...
...
echo.
echo [3/5] Descargando dependencias del proyecto...
"%DOTNET_CMD%" restore "GeneradorAnexos.WinUI.sln"
if errorlevel 1 goto :fallo

echo.
echo [4/5] Ejecutando pruebas automaticas...
"%DOTNET_CMD%" run --project "tests\GeneradorAnexos.Domain.Tests\GeneradorAnexos.Domain.Tests.csproj" -c Release --no-restore
if errorlevel 1 goto :fallo
"%DOTNET_CMD%" run --project "tests\OrderPdfReader.Regression\OrderPdfReader.Regression.csproj" -c Release --no-restore
if errorlevel 1 goto :fallo

echo.
echo [5/5] Generando el ejecutable autocontenido...
