...
public sealed class EstadoCompartido
{
    ...
    public string Oficina => _oficina;
    public event EventHandler<CambioCompartido>? OficinaCambiada;

    public void EstablecerOficina(string? texto, object? origen = null)
    {
        var valor = texto ?? string.Empty;
        if (valor == _oficina) return;
        _oficina = valor;
        OficinaCambiada?.Invoke(this, new CambioCompartido(valor, origen));
    }
