...
/// texto completo del párrafo, editando solo los nodos del marcador. El valor
/// hereda el formato del primer carácter del nombre de la variable.
/// </remarks>
public static class DocxTemplateEngine
{
    ...
    /// <summary>
    /// Sustituye solo los caracteres del marcador, incluso si Word los reparte
    /// entre varios runs. Conserva el formato del nombre de la variable y los
    /// nodos circundantes (texto, saltos, tabulaciones, imágenes y marcadores).
    /// </summary>
    internal static void SustituirEnParrafo(
        ...
        ISet<string> noResueltos)
    {
        var segmentos = new List<(Text Nodo, int Inicio, int Longitud)>();
        var original = new StringBuilder();
        foreach (var nodo in parrafo.Descendants<OpenXmlElement>())
        {
            // Los párrafos de cuadros de texto se procesan por separado.
            if (nodo.Ancestors<Paragraph>().FirstOrDefault() != parrafo) continue;
            if (nodo is Text texto && texto.Parent is Run)
            {
                segmentos.Add((texto, original.Length, texto.Text.Length));
                original.Append(texto.Text);
            }
            else if (nodo is Break or TabChar or CarriageReturn or FieldChar or Drawing)
            {
                original.Append('\0'); // No unir marcadores a través de contenido visible.
            }
        }

        // De derecha a izquierda: las posiciones de las coincidencias anteriores
        // siguen siendo válidas aunque el reemplazo tenga otra longitud.
        foreach (Match match in Marcador.Matches(original.ToString()).Cast<Match>().Reverse())
        {
            var valor = Resolver(match, contexto, noResueltos);
            var fin = match.Index + match.Length;
            var donante = match.Groups[1].Index;
            foreach (var segmento in segmentos)
            {
                var desde = Math.Max(match.Index, segmento.Inicio) - segmento.Inicio;
                var hasta = Math.Min(fin, segmento.Inicio + segmento.Longitud) - segmento.Inicio;
                if (desde >= hasta) continue;
                var insercion = donante >= segmento.Inicio && donante < segmento.Inicio + segmento.Longitud
                    ? valor : string.Empty;
                var texto = segmento.Nodo.Text;
                ReescribirNodo(segmento.Nodo, texto[..desde] + insercion + texto[hasta..]);
            }
        }
    }

    private static void ReescribirNodo(Text nodo, string texto)
    {
        var lineas = texto.Replace("\r\n", "\n", StringComparison.Ordinal).Split('\n');
        nodo.Text = lineas[0];
        nodo.Space = SpaceProcessingModeValues.Preserve;
        OpenXmlElement anterior = nodo;
        foreach (var linea in lineas.Skip(1))
        {
            var salto = new Break();
            anterior.InsertAfterSelf(salto);
            var siguiente = new Text(linea) { Space = SpaceProcessingModeValues.Preserve };
            salto.InsertAfterSelf(siguiente);
            anterior = siguiente;
        }
    }
