...
public sealed class SqliteRecordRepository : IRecordRepository
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        await using var leerVersion = conexion.CreateCommand();
        leerVersion.CommandText = "PRAGMA user_version;";
        var versionActual = Convert.ToInt32(
            await leerVersion.ExecuteScalarAsync(cancellationToken), CultureInfo.InvariantCulture);
        if (versionActual > 2)
        {
            throw new InvalidDataException("Estos registros requieren una versión más reciente del programa.");
        }
        ...
        // Una base ya migrada no vuelve a admitir texto plano al reiniciar.
        var datosHeredadosMigrados = versionActual < 2 &&
            await MigrarRegistrosHeredadosAsync(conexion, cancellationToken);
