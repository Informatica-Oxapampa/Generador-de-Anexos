...
public sealed class TablaPagos : UserControl
{
    ...
    public TablaPagos()
    {
        ...
        botonDistribuir.Click += (_, _) => AlinearYDistribuir();
    ...
    /// <summary>
    /// Registra cuántos pagos debería tener el plan sin modificar las filas
    /// importadas. Permite mostrar fielmente un registro incompleto y repararlo
    /// después, de forma explícita, con «Distribuir 100%».
    /// </summary>
    public void EstablecerCantidadEsperada(int cantidad)
        => _cantidadEsperada = Math.Max(0, cantidad);

    private void AlinearYDistribuir()
    {
        EstablecerCantidad(_cantidadEsperada);
        Distribuir();
    }
