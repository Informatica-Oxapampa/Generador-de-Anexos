...
public sealed partial class CampoCombo : UserControl, ICampo
{
    ...
    private bool _ajustandoLongitud;
    ...
    public static readonly DependencyProperty MaxLengthProperty =
        DependencyProperty.Register(nameof(MaxLength), typeof(int), typeof(CampoCombo),
            new PropertyMetadata(500, (d, _) =>
                ((CampoCombo)d).AplicarLongitudMaxima()));
    ...
    public void EstablecerValor(string? texto)
    {
        ...
        if (Editable)
        {
            Sugerencias.Text = Recortar(texto);
        }
        else
        {
    ...
    {
        if (_ajustandoLongitud)
        {
            return;
        }

        var texto = AplicarLongitudMaxima();
        if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
        {
            sender.ItemsSource = Filtrar(texto);
        }
    ...
    private string AplicarLongitudMaxima()
    {
        var texto = Sugerencias.Text ?? string.Empty;
        var ajustado = Recortar(texto);
        if (!string.Equals(texto, ajustado, StringComparison.Ordinal))
        {
            _ajustandoLongitud = true;
            try
            {
                Sugerencias.Text = ajustado;
            }
            finally
            {
                _ajustandoLongitud = false;
            }
        }
    ...
    private string Recortar(string? texto)
    {
        ...
        var limite = Math.Max(1, MaxLength);
