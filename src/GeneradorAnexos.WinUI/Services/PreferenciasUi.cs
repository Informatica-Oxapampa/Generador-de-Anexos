...
public sealed class PreferenciasUi
{
    ...
    public PreferenciasUi() : this(Path.Combine(RutaCarpeta, "preferencias.json")) { }

    internal PreferenciasUi(string ruta)
    {
        _ruta = ruta;
        try
        {
        ...
        {
            _datos = Cargar();
            // Migración automática: solo desaparecen las claves de pruebas antiguas.
            // El historial de la etapa actual y el resto de preferencias se conservan.
            var migrar = _datos.Remove("version_mas_alta_vista");
            migrar |= _datos.Remove("version_omitida");
            if (migrar) Guardar();
        }
    }
    ...
    // Identidad estable del reinicio autorizado. No cambiar en versiones futuras:
    // aísla el historial previo a 1.0.0 y mantiene la protección de retroceso.
    private const string ClaveHistorial = "mpo_reinicio_20260908_";
    ...
    {
        get => LeerTexto(ClaveHistorial + "version_omitida");
        set => Escribir(ClaveHistorial + "version_omitida", value);
    }
    ...
    {
        get => LeerTexto(ClaveHistorial + "version_mas_alta_vista");
        set => Escribir(ClaveHistorial + "version_mas_alta_vista", value);
    }
