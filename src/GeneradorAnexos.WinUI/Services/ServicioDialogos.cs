...
{
    private static readonly SemaphoreSlim DialogoActivo = new(1, 1);

    // WinUI solo admite un ContentDialog abierto por ventana.
    public static async Task<ContentDialogResult> MostrarDialogoAsync(
        ContentDialog dialogo, Func<Task>? durante = null)
    {
        await DialogoActivo.WaitAsync();
        try
        {
            var raiz = RaizActiva;
            if (raiz is null) return ContentDialogResult.None;
            dialogo.XamlRoot = raiz;
            dialogo.RequestedTheme = ServicioTema.TemaEfectivo;
            var mostrando = dialogo.ShowAsync();
            if (durante is not null)
            {
                try { await durante(); }
                finally { dialogo.Hide(); await mostrando; }
                return ContentDialogResult.None;
            }
            return await mostrando;
        }
        finally
        {
            DialogoActivo.Release();
        }
    }
    ...
    public static async Task<bool> PreguntarSiNoAsync(
        ...
        string accion = "Confirmar", string cancelar = "Cancelar")
    {
        var raiz = RaizActiva;
        if (raiz is null)
        {
    ...
    public static async Task<ResultadoExito> MostrarExitoAsync(string ruta)
    {
        ...
        var eleccion = await MostrarDialogoAsync(dialogo);
        ...
        string textoDescartar)
    {
        ...
        if (raiz is null)
        {
            return RespuestaCambios.Cancelar;
        }
        ...
        return await MostrarDialogoAsync(dialogo) switch
        {
    ...
    public static async Task<string?> PedirTextoAsync(
        string titulo, string etiqueta, string valorInicial, string accion = "Guardar registro")
    {
        var raiz = RaizActiva;
        if (raiz is null)
        {
        ...
        var dialogo = new ContentDialog
        {
            ...
            Content = contenido,
            PrimaryButtonText = accion,
            IsPrimaryButtonEnabled = !string.IsNullOrWhiteSpace(valorInicial),
            CloseButtonText = "Cancelar",
            DefaultButton = ContentDialogButton.Primary,
        };

        caja.TextChanged += (_, _) => dialogo.IsPrimaryButtonEnabled = !string.IsNullOrWhiteSpace(caja.Text);
        return await MostrarDialogoAsync(dialogo) == ContentDialogResult.Primary
            ? caja.Text.Trim()
            : null;
    }

    /// <summary>Muestra todos los errores sin recortar formularios con muchos datos pendientes.</summary>
    public static async Task MostrarErroresValidacionAsync(IReadOnlyList<string> errores)
    {
        await MostrarDialogoAsync(new ContentDialog
        {
            Title = "Revise estos datos antes de generar",
            ...
            CloseButtonText = "Volver al formulario",
        });
    }
