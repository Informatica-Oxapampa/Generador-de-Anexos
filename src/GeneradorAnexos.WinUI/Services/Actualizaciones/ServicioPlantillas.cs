...
public static class ServicioPlantillas
{
    ...
    {
        // Recupera el intervalo entre los dos renombrados si Windows se cerró
        // después de apartar las plantillas anteriores y antes de promover las nuevas.
        if (!Directory.Exists(Carpeta))
        {
            try
            {
                var raiz = Path.GetDirectoryName(Carpeta)!;
                var respaldo = Directory.Exists(raiz)
                    ? Directory.GetDirectories(raiz, "plantillas.anterior-*")
                        .OrderByDescending(Directory.GetLastWriteTimeUtc).FirstOrDefault(ConjuntoValido)
                    : null;
                if (respaldo is not null)
                {
                    Directory.Move(respaldo, Carpeta);
                    Registro.Info("TEMPLATES_RECOVERED_AFTER_INTERRUPTION");
                }
            }
            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
            {
                Registro.Error("TEMPLATES_RECOVERY_FAILED", ex);
            }
        }
