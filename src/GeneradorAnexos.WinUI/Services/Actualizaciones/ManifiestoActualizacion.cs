...
public sealed class PaqueteActualizacion
{
    ...
    public bool EsValido(out VersionSemantica version)
    {
        if (!VersionSemantica.TryParse(Version, out version) ||
            ...
            string.IsNullOrWhiteSpace(Sha256) || Sha256.Trim().Length != 64 ||
            !EsUrlPublicacionOficial(Url) ||
            !DateTime.TryParseExact(
                Fecha, "yyyy-MM-dd", CultureInfo.InvariantCulture,
                DateTimeStyles.None, out _) ||
            Archivos is null || Notas is null || Archivos.Count > 100 || Notas.Count > 50 ||
            Archivos.Any(x => string.IsNullOrWhiteSpace(x) || x.Length > 160) ||
            Notas.Any(x => x is null || x.Length > 500))
        {
            return false;
        }
    ...
    public static bool EsDescargaPermitida(string url)
    {
        if (!Uri.TryCreate(url, UriKind.Absolute, out var direccion)
            ...
            || !direccion.IsDefaultPort || !string.IsNullOrEmpty(direccion.UserInfo))
        {
            return false;
        }
    ...
    {
        if (!EsDescargaPermitida(url)) return false;
        var direccion = new Uri(url, UriKind.Absolute);
        var prefijo = $"/{ConfiguracionActualizaciones.Propietario}/{ConfiguracionActualizaciones.Repositorio}/releases/download/";
        return string.Equals(direccion.Host, "github.com", StringComparison.OrdinalIgnoreCase) &&
               direccion.AbsolutePath.StartsWith(prefijo, StringComparison.OrdinalIgnoreCase) &&
               direccion.AbsolutePath[prefijo.Length..].Split('/').Length == 2 &&
               !direccion.AbsolutePath.Contains('%') &&
               string.IsNullOrEmpty(direccion.Query) && string.IsNullOrEmpty(direccion.Fragment);
    }
}
