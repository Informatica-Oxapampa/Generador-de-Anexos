...
public static class ServicioActualizaciones
{
    ...
    /// </summary>
    public static Task<ResultadoComprobacion> ComprobarAsync(CancellationToken cancelacion)
        => ComprobarAsync(Cliente.Value, cancelacion);

    internal static async Task<ResultadoComprobacion> ComprobarAsync(
        HttpClient cliente, CancellationToken cancelacion)
    {
        try
        {
            ...
            using var respuesta = await EnviarConRedireccionesSegurasAsync(
                ...
                limite.Token, cliente).ConfigureAwait(false);
            if (respuesta.StatusCode == HttpStatusCode.NotFound)
            {
                return ResultadoComprobacion.Fallida(
                    "Todavía no hay una actualización publicada con su archivo update.json. " +
                    "La OTI debe publicar una Release completa; subir código no publica una versión.");
            }
            ...
            if (!EsManifiestoValido(manifiesto))
            {
                ...
                return ResultadoComprobacion.Fallida(
                    "La publicación no es compatible con esta edición o está incompleta. La OTI debe publicar una versión con el nuevo flujo de actualizaciones.");
            }
            ...
            if (RequiereVersionIntermedia(manifiesto.App!))
            {
                ...
                return ResultadoComprobacion.Fallida(
                    ...
                    "Solicite a la OTI el instalador correspondiente.");
            }
            ...
            // Se conserva la versión más alta recibida del repositorio oficial.
    ...
    private static bool EsManifiestoReciente(ManifiestoActualizacion manifiesto)
    {
        ...
        if (Constantes.EsVersionPrueba) return true;
    ...
    private static void RegistrarVersionConfiable(PaqueteActualizacion paquete)
    {
        ...
        var preferencias = new PreferenciasUi();
        if (Constantes.EsVersionPrueba || !VersionSemantica.TryParse(preferencias.VersionMasAltaVista, out var maxima) || publicada > maxima)
        {
        ...
        HttpClient? cliente = null)
    {
        ...
        for (var salto = 0; salto <= ConfiguracionActualizaciones.MaximoRedirecciones; salto++)
        {
            ...
            var respuesta = await (cliente ?? Cliente.Value).SendAsync(peticion, opcion, cancelacion)
                .ConfigureAwait(false);
    ...
    internal static void LimpiarDescargasAnteriores(string excepto)
    {
        try
        {
            if (!Directory.Exists(CarpetaDescargas)) return;
            foreach (var archivo in Directory.EnumerateFiles(CarpetaDescargas))
            {
                var nombre = Path.GetFileName(archivo);
                var propio = System.Text.RegularExpressions.Regex.IsMatch(nombre,
                    @"^(GeneradorAnexos-[0-9]+\.[0-9]+\.[0-9]+-Setup\.exe|plantillas-[0-9]+\.[0-9]+\.[0-9]+\.zip)$");
                if (propio && !string.Equals(archivo, excepto, StringComparison.OrdinalIgnoreCase) &&
                    (File.GetAttributes(archivo) & FileAttributes.ReparsePoint) == 0)
                    Descartar(archivo);
            }
        }
        catch (IOException) { }
...
public sealed class ResultadoComprobacion
{
    ...
    public PaqueteActualizacion? AppPendiente(out VersionSemantica version)
    {
        ...
        return DebeInstalar(ServicioActualizaciones.VersionInstalada, publicada, Constantes.EsVersionPrueba) ? app : null;
    }

    internal static bool DebeInstalar(VersionSemantica instalada, VersionSemantica publicada, bool esPrueba)
        => publicada > instalada || esPrueba;
