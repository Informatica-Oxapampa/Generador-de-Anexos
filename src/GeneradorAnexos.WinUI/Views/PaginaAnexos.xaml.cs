...
public sealed partial class PaginaAnexos : UserControl
{
    ...
    /// <summary>Refleja si el formulario actual ya puede generar Anexos válidos.</summary>
    public void RefrescarEstadoGeneracion()
    {
        var errores = DisponibilidadDocumentos.ErroresAnexos(_ventana?.RecolectarBorrador());
        var ocupado = SemaforoGeneracion.CurrentCount == 0;
        // Permitir validar desde el botón; nunca dejar un bloqueo sin explicación.
        BotonGenerarAnexos.IsEnabled = !ocupado;
        EstadoGeneracionAnexos.Text = ocupado ? "Generación en curso…"
            : errores.Count == 0 ? "Anexos listos para generar"
            : "Revise: " + errores[0];
        var detalle = errores.Count == 0 ? "Generar el documento Word"
            : "Al generar se indicarán los datos que debe corregir:\n" + string.Join("\n", errores);
        ToolTipService.SetToolTip(BotonGenerarAnexos, detalle);
        ToolTipService.SetToolTip(EstadoGeneracionAnexos, detalle);
    }
    ...
    public void ConectarEstado()
    {
        ...
        CampoOficina.Cambiado += (_, _) => _estado.EstablecerOficina(CampoOficina.Valor, CampoOficina);
        _estado.OficinaCambiada += (_, cambio) =>
        {
            if (!ReferenceEquals(cambio.Origen, CampoOficina))
                CampoOficina.EstablecerValorSilencioso(cambio.Texto);
        };
    ...
    private IEnumerable<ICampo> CamposObligatorios()
        => CamposFormulario().Append(CampoOficina).Where(campo => campo.Obligatorio);
    ...
    {
        var errores = DisponibilidadDocumentos.ErroresAnexos(_ventana?.RecolectarBorrador());
        // La misma validación alimenta el estado y la generación. Los controles
        // se marcan para facilitar la corrección, sin un segundo veto diferente.
        _ = ValidarTodo();
        if (errores.Count > 0)
        {
            EnfocarPrimerInvalido();
            await ServicioDialogos.MostrarErroresValidacionAsync(errores);
            return;
        }
