...
public sealed partial class PaginaTdr : UserControl
{
    ...
    /// <summary>Refleja si el formulario actual ya puede generar un TDR válido.</summary>
    public void RefrescarEstadoGeneracion()
    {
        var errores = DisponibilidadDocumentos.ErroresTdr(ExportarEstado());
        var ocupado = SemaforoGeneracion.CurrentCount == 0;
        // Permitir validar desde el botón; nunca dejar un bloqueo sin explicación.
        BotonGenerarTdr.IsEnabled = !ocupado;
        EstadoGeneracionTdr.Text = ocupado ? "Generación en curso…"
            : errores.Count == 0 ? "TDR listo para generar"
            : "Revise: " + errores[0];
        var detalle = errores.Count == 0 ? "Generar el documento Word"
            : "Al generar se indicarán los datos que debe corregir:\n" + string.Join("\n", errores);
        ToolTipService.SetToolTip(BotonGenerarTdr, detalle);
        ToolTipService.SetToolTip(EstadoGeneracionTdr, detalle);
    }
