...
public sealed partial class VentanaPrincipal : Window
{
    ...
    private static readonly (string Titulo, string Descripcion)[] Secciones =
    {
        ...
        ("Registros guardados",
         "Abra y reutilice datos guardados sin modificar el registro original hasta que elija actualizarlo."),
        ("Configuración",
    ...
    /// Un segundo es suficiente para que el aviso y los botones se actualicen
    /// de forma perceptiblemente inmediata, y el coste
    ...
    private bool _actualizandoSelectorFecha;
    ...
    /// </summary>
    /// <remarks>
    /// Al reutilizar un registro guardado se establece en el día actual. Las
    /// fechas históricas de creación y actualización viven en la base de datos
    /// y no se cambian al cargar ni al generar un archivo.
    /// </remarks>
    ...
    private void SincronizarSelectorFecha()
    {
        _actualizandoSelectorFecha = true;
        try
        {
            SelectorFechaDocumento.Date = new DateTimeOffset(new DateTime(
                _fechaDocumento.Year,
                _fechaDocumento.Month,
                _fechaDocumento.Day,
                0, 0, 0,
                DateTimeKind.Local));
        }
        ...
        {
            _actualizandoSelectorFecha = false;
        }
    }

    private void AlCambiarFechaDocumento(
        CalendarDatePicker sender,
        CalendarDatePickerDateChangedEventArgs args)
    {
        if (_actualizandoSelectorFecha)
        {
            return;
        }

        if (args.NewDate is not { } nuevaFecha)
        {
            SincronizarSelectorFecha();
            return;
        }

        FechaDocumento = DateOnly.FromDateTime(nuevaFecha.Date);
        Registro.Info("DOCUMENT_DATE_CHANGED");
        RefrescarAvisoDeCambios();
    }
    ...
    {
        var hayCambios = HayCambiosSinGuardar();
        var hayRegistroActivo = PaginaUsuariosVista.IdRegistroCargado is not null;

        ElementoGuardar.IsEnabled = hayRegistroActivo && hayCambios;
        ElementoGuardar.Content = !hayRegistroActivo
            ? "Guardar"
            : hayCambios ? "Actualizar registro" : "Sin cambios";
        ToolTipService.SetToolTip(
            ElementoGuardar,
            !hayRegistroActivo
                ? "Use «Nuevo registro» para crear y nombrar un registro."
                : hayCambios
                    ? "Actualizar el registro activo con los cambios actuales (Ctrl+S)."
                    : "El registro activo ya contiene estos datos.");

        PaginaTdrVista.RefrescarEstadoGeneracion();
        PaginaAnexosVista.RefrescarEstadoGeneracion();
        ...
        RegistroActivoEtiqueta.Text = !hayRegistroActivo
            ? hayCambios ? "BORRADOR SIN REGISTRO" : "SIN REGISTRO ACTIVO"
            : hayCambios ? "REGISTRO ACTIVO · SIN GUARDAR" : "REGISTRO ACTIVO";
    }
    ...
    /// <summary>
    /// Finaliza el cierre solo después de que el instalador verificado haya sido
    ...
    ///   <item>Se pospone la instalación si hay trabajo sin guardar.</item>
    /// </list>
    /// </remarks>
    private void ProgramarComprobacionActualizaciones()
    {
        ...
        temporizador.Tick += async (_, _) =>
        {
            ...
            if (HayCambiosSinGuardar())
            {
                temporizador.Interval = TimeSpan.FromMinutes(1);
                temporizador.Start();
            }
        };
        temporizador.Start();
    ...
    private static async Task ComprobarActualizacionesEnSegundoPlanoAsync(PreferenciasUi preferencias)
    {
        try
        {
            ...
            await AsistenteActualizacion.OfrecerAsync(resultado, instalarDirectamente: true, enSegundoPlano: true);
        }
        catch (Exception excepcion)
        {
